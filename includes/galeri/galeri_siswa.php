...
$query = "SELECT * FROM galeri_siswa ORDER BY id DESC";
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $galeri_items[] = [
            'img' => '/Project_SMPN3/admin/dist/galeri/galeri_siswa/uploads/' . $row['filename'], // sesuaikan pathnya ya' . $row['filename'],  // sesuaikan pathnya ya
            'caption' => $row['caption']
        ];
    }
} else {
...
<body>
    ...
    <!-- Banner -->
    <?php
        $bannerImage = 'assets/img/siswa.jpg';
